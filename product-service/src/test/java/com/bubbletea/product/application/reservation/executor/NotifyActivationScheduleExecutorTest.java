...
class NotifyActivationScheduleExecutorTest {

    @InjectMocks
    private NotifyActivationScheduleExecutor notifyActivationScheduleExecutor;
    ...
    @Test
    @DisplayName("supports()는 NOTIFY_ACTIVATION_SCHEDULE 타입을 반환한다.")
    void supports_returnsNotifyActivationScheduleType() {
        // when
        ReservationCommandType result = notifyActivationScheduleExecutor.supports();

        // then
        assertThat(result).isEqualTo(ReservationCommandType.NOTIFY_ACTIVATION_SCHEDULE);
    }

    @Test
    @DisplayName("활성화 일정을 정상적으로 알림으로 발송하고 이력을 저장한다.")
    void execute_success() {
        ...
        payload.put(ACTIVATION_DATE, activationDate.toString());
        ...
        // when
        notifyActivationScheduleExecutor.execute(reservation);

        // then
        then(productScheduleNotificationService).should()
            .notifyActivationSchedule(eq(reservationId), eq(productName), eq(activationDate));
